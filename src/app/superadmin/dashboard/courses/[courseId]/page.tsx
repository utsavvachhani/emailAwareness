...
export default function CourseModulesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Check video limit if adding a new video or updating to a video
    if (planInfo && form.type === 'video') {
      const isCurrentlyVideo = editModule && editModule.type === 'video';
      if (!isCurrentlyVideo && videoModules.length >= planInfo.video_limit) {
        toast.error(`Your plan allows max ${planInfo.video_limit} video(s) per course.`);
        return;
      }
    }

    // Check total module limit if adding new
    if (!editModule && planInfo && modules.length >= planInfo.module_limit) {
      toast.error(`Your plan allows max ${planInfo.module_limit} modules per course.`);
      return;
    }
    ...
    try {
      const payload = {
        ...
        image_url: form.type === 'docs' ? form.image_url : null,
      };

      if (editModule) {
        const res = await adminUpdateModule(editModule.id.toString(), {
          ...payload,
          order_index: editModule.order_index
        });
        if (res.data.success) {
          toast.success("Module updated");
          setModules(prev => prev.map(m => m.id === editModule.id ? res.data.module : m));
          setEditModule(null);
          setShowForm(false);
        }
      } else {
        const res = await adminCreateModule(courseId, {
          ...payload,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await adminDeleteModule(id.toString());
      if (res.data.success) {
  ...
  const toggleStatus = async (m: Module) => {
    ...
    try {
      const res = await adminUpdateModule(m.id.toString(), { ...m, status: newStatus });
      if (res.data.success) {
        setModules(prev => prev.map(mod => mod.id === m.id ? res.data.module : mod));
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check plan limits for video
    if (isVideo && planInfo && videoModules.length >= planInfo.video_limit && (!form.video_url || editModule?.video_url !== form.video_url)) {
      toast.error(`Your plan allows max ${planInfo.video_limit} video(s) per course.`);
      return;
    }
              ...
              {form.type === 'video' ? (
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex items-start gap-3">
                     ...
                     <div>
                       <p className="text-xs font-bold text-blue-900">Setting up a document module...</p>
                       <p className="text-[10px] text-blue-700/80 font-medium mt-1 leading-relaxed">
                          After creation, click on the module in the list below to build your rich document with headings and images.
                       </p>
                     </div>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4 bg-card border border-border rounded-2xl border-dashed">
              ...
              <p className="text-sm font-medium text-muted-foreground">Organizing modules...</p>
            </div>
          ) : modules.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center bg-card border border-border rounded-2xl border-dashed">
              ...
              <h3 className="text-lg font-bold">Your course is currently empty</h3>
              <p className="text-sm text-muted-foreground max-w-sm mt-2 mb-8 leading-relaxed">
                Add modules to create a structured learning path for your employees. Each module can contain text content and training videos.
              </p>
              <button
            ...
            <div className="grid grid-cols-1 gap-4">
              {modules.map((m, idx) => (
                <div 
                  ...
                  onClick={() => router.push(`/superadmin/dashboard/courses/${courseId}/modules/${m.id}`)}
                >
