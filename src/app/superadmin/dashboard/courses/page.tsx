...
import {
  ...
  AlertCircle, BarChart3, Building2, User, Calendar, RefreshCcw, Search,
} from "lucide-react";
...
function CourseDetailModal({ course, onClose, onApprove, onReject, processing }: DetailModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
        ...
        {course.status === "approved" && (
          <div className="px-6 py-4 border-t border-border bg-emerald-500/5">
            <div className="flex items-center justify-center gap-2 text-sm font-semibold text-emerald-700">
              <CheckCircle2 className="w-4 h-4" /> This course has been approved
            </div>
          </div>
        )}
      </div>
...
function CourseRow({ course, onView, onApprove, processing }: CourseRowProps) {
  const diff = DIFF_CFG[course.difficulty] ?? DIFF_CFG.medium;
  const stat = STATUS_CFG[course.status] ?? STATUS_CFG.pending;
  const StatusIcon = stat.icon;
