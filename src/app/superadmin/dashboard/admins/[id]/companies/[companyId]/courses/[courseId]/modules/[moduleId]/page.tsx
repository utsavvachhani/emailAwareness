...
import {
  ...
  Plus, Search, Filter, Trash2, MoreVertical, LayoutList, Eye, EyeOff, Edit3, Save, AlignLeft, BookOpen, Image as ImageIcon, Target
} from "lucide-react";
...
const EditorJSComponent = dynamic(() => import("@/components/EditorJSComponent"), {
  ssr: false,
  loading: () => <div className="h-40 flex items-center justify-center bg-secondary/20 rounded-2xl animate-pulse text-xs font-bold text-muted-foreground uppercase tracking-widest italic text-center">Initializing Editor Viewport...</div>
});
  ...
  if (loading && !module) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-8">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground animate-pulse italic text-center">Initalizing Oversight Editor...</p>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between border-b pb-6 border-border">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => { fetchDetails(); toast.success("Data refreshed!"); }}
            disabled={loading}
        ...
        <form onSubmit={handleSave} className="bg-card border border-border rounded-3xl overflow-hidden shadow-2xl p-8 md:p-12 space-y-10">
          ...
          {form.type === 'video' ? (
            <div className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  rows={6}
                  placeholder="Describe the training video..."
                  className="w-full p-6 rounded-2xl border border-input bg-background outline-none text-sm"
                />
              </div>
            ...
            <div className="space-y-10">
              <div className="bg-purple-50 border border-purple-100 p-8 rounded-3xl">
                ...
                <p className="text-sm text-purple-800/70 mt-1">Refine institutional MCQ assessments for this instructional module.</p>
              </div>
              <div className="grid grid-cols-1 gap-6">
                {(() => {
                  let quizData: any = { questions: [] };
                  try { if (form.contentextra) quizData = JSON.parse(form.contentextra); } catch { }
                        ...
                        {Array.from({ length: 4 }).map((__, j) => (
                          <div key={j} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${qList[i]?.answer === j ? 'bg-purple-600 border-purple-600 text-white' : 'bg-secondary/30 border-border'}`}>
                            ...
                            <input
                              ...
                              className="flex-1 bg-transparent border-none outline-none text-xs font-medium placeholder:text-muted-foreground/30"
                            />
                          </div>
                        ))}
                      </div>
            ...
            <div className="space-y-10">
              ...
              <div className="min-h-[400px] p-6 rounded-3xl bg-secondary/10 border border-border shadow-inner">
                <EditorJSComponent 
          ...
          <div className="flex gap-4 pt-8 border-t border-border">
            <button
              ...
              className="flex-1 h-16 rounded-2xl bg-blue-600 text-white font-black text-sm uppercase tracking-widest hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-2xl shadow-blue-600/20"
            >
              {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
              Publish Administrative Override
            </button>
          </div>
        </form>
      ) : (
            ...
            {module.type === 'video' ? (
              <div className="space-y-8">
                <div className="aspect-video rounded-3xl overflow-hidden bg-black border border-border shadow-2xl">
                  {module.video_url ? <video src={module.video_url} controls className="w-full h-full" /> : <div className="h-full flex items-center justify-center text-white/20">No Video Asset</div>}
                </div>
                <div className="bg-card border border-border rounded-3xl p-8">
                  <h2 className="text-xl font-black mb-4">Training Overview</h2>
                  <p className="text-foreground/80 leading-relaxed whitespace-pre-wrap">{module.content || "No description."}</p>
                </div>
              </div>
            ) : module.type === 'quiz' ? (
              <div className="space-y-8">
                <div className="bg-purple-600 rounded-[40px] p-10 text-white shadow-xl relative overflow-hidden">
                  <div className="relative z-10">
                    ...
                    <p className="opacity-80">Refining institutional training retention with a 5-question audit.</p>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {(() => {
                    try {
                      ...
                      if (questions.length === 0) return <div className="p-20 text-center italic text-muted-foreground border-2 border-dashed border-border rounded-3xl">No assessment configured.</div>;
                    ...
                    } catch { return <div className="p-10 text-center italic text-muted-foreground border-2 border-dashed border-border rounded-3xl">Data integrity audit failed.</div>; }
                  })()}
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-3xl p-8 shadow-sm space-y-6">
              ...
              <div className="flex items-center justify-between border-t border-border pt-4">
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Type</span>
                <span className="text-xs font-black capitalize flex items-center gap-2">
                  {module.type === 'video' ? <Video className="w-3.5 h-3.5 text-blue-600" /> : module.type === 'quiz' ? <Target className="w-3.5 h-3.5 text-purple-600" /> : <FileText className="w-3.5 h-3.5 text-orange-600" />}
                  {module.type}
                </span>
              </div>
            </div>
            ...
            <div className="bg-foreground rounded-3xl p-8 text-background shadow-2xl relative overflow-hidden group">
               <ShieldCheck className="absolute -right-5 -bottom-5 w-32 h-32 opacity-10 group-hover:scale-120 transition-transform duration-700" />
               <div className="relative z-10 flex flex-col gap-6">
                  <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                    <BookOpen className="w-6 h-6" />
                  </div>
                  <div>
                    <h4 className="text-lg font-black tracking-tighter uppercase tracking-tight">Oversight Strategy</h4>
                    <p className="text-[10px] opacity-60 mt-2 leading-relaxed">This node is part of a global compliance curriculum. Superadmin overrides are logged and audited.</p>
                  </div>
               </div>
            </div>
          </div>
