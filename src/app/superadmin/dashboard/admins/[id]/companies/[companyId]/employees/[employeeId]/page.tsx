...
import { 
  ...
  Loader2, Search, Filter, RefreshCw, Phone,
  TrendingUp, Award, LayoutGrid, ShieldCheck,
  ChevronRight, Calendar, Info
} from "lucide-react";
import { toast } from "sonner";
import { superadminGetEmployeeProgress } from "@/api";
...
export default function SuperadminEmployeeDetailPage() {
  ...
  const fetchProgress = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await superadminGetEmployeeProgress(employeeId);
      if (res.data.success) {
        setData(res.data);
      }
    } catch {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-sm font-medium text-muted-foreground animate-pulse tracking-widest text-center uppercase">Calibrating Global Metrics...</p>
      </div>
    );
  }
  ...
  if (!data || !stats) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <h2 className="text-lg font-bold">Node not found in global registry</h2>
        <button onClick={() => router.back()} className="mt-4 text-sm text-blue-600 font-bold hover:underline flex items-center gap-2">
          <ArrowLeft className="w-4 h-4" /> Return to Enterprise Inventory
        </button>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500 print:p-0 print:space-y-4 print:mx-0 print:max-w-none">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 print:hidden">
        <div className="space-y-4">
          ...
          >
            ...
            Back to Enterprise Registry
          </button>
          
          <div className="flex items-center gap-6">
            <div className="w-24 h-24 rounded-[32px] bg-gradient-to-br from-purple-600 to-indigo-700 flex items-center justify-center shadow-2xl shadow-purple-500/20 border-2 border-white/10 ring-4 ring-purple-500/5">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-500/20 bg-blue-500/10 text-blue-600`}>
              ...
              <div className="flex flex-wrap items-center gap-y-2 gap-x-4 mt-3">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground bg-secondary/50 px-3 py-1.5 rounded-xl border border-border/50">
                  <LayoutGrid className="w-3.5 h-3.5 text-orange-600" /> Organization ID: {employee.company_id}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="hidden print:block border-b-2 border-foreground pb-8 mb-8">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-4xl font-black uppercase tracking-tighter">CyberShield Enterprise Report</h1>
            <p className="text-sm font-bold text-muted-foreground mt-1">GLOBAL CURRICULUM PERFORMANCE & COMPLIANCE VERIFICATION</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-10 mt-12">
          ...
          <div className="grid grid-cols-3 gap-4">
             ...
             <div className="p-4 bg-muted rounded-2xl border border-border">
                <p className="text-[8px] font-black uppercase text-muted-foreground tracking-tighter">Global Score</p>
                <div className="text-xl font-black text-blue-600">{stats.globalAvgScore ?? 0}%</div>
             </div>
          </div>
        </div>
      </div>

      {/* ── Global Metrics Wavefront ─────────────────────────────── */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 print:grid-cols-4">
        {[
          { label: "Enterprise Courses", value: stats.totalAssigned, icon: BookOpen, color: "text-blue-600", bg: "bg-blue-500/5" },
          { label: "Global Completions", value: stats.totalCompleted, icon: Award, color: "text-emerald-600", bg: "bg-emerald-500/5" },
          { label: "Logic Retention Index", value: `${stats.globalAvgScore ?? 0}%`, icon: Target, color: "text-purple-600", bg: "bg-purple-500/5" },
          { label: "System Contribution", value: stats.globalModulesCompleted * 10, icon: TrendingUp, color: "text-orange-500", bg: "bg-orange-500/5" },
        ].map(stat => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 print:flex print:flex-col">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between border-b border-border pb-4">
            ...
            <span className="text-[10px] font-black px-3 py-1 bg-secondary rounded-lg">ENTERPRISE AUDIT</span>
          </div>
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="text-lg font-black tracking-tight underline decoration-purple-500/30 decoration-2 underline-offset-4">{c.title}</h4>
                      {c.isCompleted && (
                        <div className="flex items-center gap-1 text-[8px] font-black uppercase tracking-widest bg-blue-600 text-white px-2 py-0.5 rounded-full shadow-lg shadow-blue-500/20">
                          <CheckCircle2 className="w-2.5 h-2.5" /> Verified Completion
                        </div>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground line-clamp-1 opacity-70 font-medium italic">{c.description || "Enterprise training node description restricted."}</p>
                  </div>
                  <div className="text-right shrink-0 ml-4">
                    <div className={`text-2xl font-black italic tracking-tighter leading-none ${c.avgScore && c.avgScore >= 80 ? 'text-blue-600' : 'text-foreground'}`}>
                      {c.avgScore !== null ? `${c.avgScore}%` : "—"}
                    </div>
                    <p className="text-[8px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Audit Metric</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    <span className="flex items-center gap-1.5 italic">
                      ...
                      Node Mastery: {c.completedCount} / {c.total_modules}
                    </span>
                  ...
                  <div className="h-2 w-full bg-secondary rounded-full overflow-hidden shadow-inner">
                    <div 
                      className={`h-full transition-all duration-1000 ease-out shadow-lg ${
                        c.progressPercent === 100 ? 'bg-gradient-to-r from-blue-600 to-indigo-600' : 'bg-gradient-to-r from-purple-600 to-indigo-600'
                      }`}
        ...
        <div className="space-y-8">
          <div className="bg-card border border-border rounded-3xl p-8 shadow-sm space-y-8 sticky top-24">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-br from-blue-500/10 to-indigo-500/10 border border-blue-500/20">
                <div className="w-12 h-12 rounded-xl bg-white/50 backdrop-blur shadow-inner flex items-center justify-center border border-blue-500/10">
                  <TrendingUp className="w-6 h-6 text-blue-600" />
                </div>
                <div>
                  <p className="text-xl font-black text-blue-950 italic tracking-tighter leading-none">{stats.globalAvgScore ?? 0}%</p>
                  <p className="text-[10px] font-black uppercase tracking-widest text-blue-900/60 mt-1">Enterprise Retention</p>
                </div>
              </div>

              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground border-b border-border/50 pb-2">Compliance Leaders</p>
              ...
              <div className="p-5 rounded-2xl bg-blue-500/5 border border-blue-500/15">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h5 className="text-[10px] font-black uppercase tracking-widest text-blue-900">Governance Insight</h5>
                    <p className="text-[10px] font-medium text-blue-800/70 mt-1 leading-relaxed">
                      This trainee represents a high-proficiency node within the organization. Audit responses indicate a superior understanding of global security frameworks.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {/* ── Global Assessment Transcript (PDF Focused) ────────────── */}
      <div className="hidden print:block space-y-6 mt-12 bg-white">
        <h3 className="text-xl font-black uppercase tracking-widest border-b-2 border-foreground pb-4">Enterprise Assessment Transcript</h3>
        <table className="w-full text-left">
           <thead className="bg-muted border-b border-border">
             <tr>
               <th className="px-4 py-3 text-[10px] font-black uppercase">Organization Node</th>
               <th className="px-4 py-3 text-[10px] font-black uppercase">Taxonomy</th>
               <th className="px-4 py-3 text-[10px] font-black uppercase">Audit Timestamp</th>
               <th className="px-4 py-3 text-[10px] font-black uppercase">KPI Metric</th>
             </tr>
           </thead>
           <tbody className="divide-y divide-border">
             {data.progress.map(p => (
               <tr key={p.id}>
                 ...
                 <td className="px-4 py-3 text-xs font-black">{p.quiz_score !== null ? `${p.quiz_score}%` : 'S-Tier Compliance'}</td>
               </tr>
             ))}
           </tbody>
        </table>
        ...
        <div className="mt-20 pt-8 border-t border-border flex justify-between items-center opacity-30 italic text-[10px]">
           <p>CyberShield Global Enterprise Portal &copy; 2026. Data verification wavefront E-099.</p>
