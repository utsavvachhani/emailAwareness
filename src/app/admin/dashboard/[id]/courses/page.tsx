...
function AssignCourseSidebar({
  ...
  useEffect(() => {
    if (open && companyId) {
      const fetchEmps = async () => {
        setLoading(true);
        try {
          const res = await adminGetEmployeesByCompany(companyId);
  ...
  const handleAssign = async () => {
    ...
    setAssigning(true);
    try {
      const res = await adminAssignCourseToEmployees(courseId, selected);
      if (res.data.success) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-card border-l border-border shadow-2xl h-full flex flex-col animate-in slide-in-from-right duration-300">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <div>
            <h2 className="font-bold text-lg">Assign Training</h2>
            <p className="text-[10px] uppercase font-black tracking-widest text-blue-600 mt-1">{courseTitle}</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-secondary"><X className="w-4 h-4" /></button>
        </div>

        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center justify-between">
             Selection ({selected.length})
             {selected.length > 0 && <button onClick={() => setSelected([])} className="text-[10px] text-blue-600 hover:underline">Clear</button>}
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-10">
              <p className="text-sm font-medium text-muted-foreground">No employees found in this entity.</p>
            </div>
          ) : (
            employees.map(emp => (
              <div 
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center gap-4 ${
                  selected.includes(emp.id) ? "border-blue-500 bg-blue-500/5 shadow-inner shadow-blue-500/10" : "border-border bg-background hover:border-blue-500/30"
                }`}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs ${
                  selected.includes(emp.id) ? "bg-blue-600 text-white" : "bg-secondary text-muted-foreground"
                }`}>
                  {emp.first_name[0]}{emp.last_name[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold truncate">{emp.first_name} {emp.last_name}</p>
                  <p className="text-[10px] text-muted-foreground truncate">{emp.email}</p>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                  selected.includes(emp.id) ? "bg-blue-600 border-blue-600 shadow-sm" : "border-border"
                }`}>
                  {selected.includes(emp.id) && <CheckCircle2 className="w-3 h-3 text-white" />}
                </div>
              </div>
            ))
          )}
        ...
        <div className="p-6 border-t border-border bg-secondary/20">
          <button 
            ...
            className="w-full h-14 rounded-2xl bg-blue-600 text-white font-bold text-sm shadow-xl shadow-blue-600/20 hover:bg-blue-500 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
          >
            {assigning ? <Loader2 className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />}
            {assigning ? "Provisioning..." : `Assign Enrollment (${selected.length})`}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {course.status === 'approved' && (
          <button
            ...
            className="flex-1 h-8 rounded-lg bg-blue-600 text-white text-[10px] font-bold uppercase tracking-widest hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-600/10"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(course => (
              <CourseCard
                ...
                onAssign={() => setAssignState({ open: true, id: course.id, title: course.title })}
