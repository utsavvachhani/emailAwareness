...
import { sendOTP, sendPasswordResetEmail, sendAdminRegistrationAlert } from "../utils/email.js";

// ─── Admin Signup ─────────────────────────────────────────────────────────────
export const signup = async (req, res) => {
  const { firstName, lastName, email, password, mobile, organization } = req.body;
  ...
  try {
    ...
    await client.query("INSERT INTO profiles (admin_id, organization, designation) VALUES ($1, $2, 'Administrator')", [user.id, organization]);

    const otp = generateOTP();
    await client.query("INSERT INTO otps (admin_id, otp, expires_at) VALUES ($1, $2, NOW() + INTERVAL '10 minutes')", [user.id, otp]);

    await client.query("COMMIT");
    sendOTP(normalizedEmail, otp).catch(() => {});
    sendAdminRegistrationAlert(`${firstName} ${lastName}`, normalizedEmail).catch(() => {});

    return res.status(201).json({
      success: true,
      message: "Admin registration successful. Please verify your email. Access pending superadmin approval.",
...
export const signin = async (req, res) => {
  ...
  try {
    ...
    if (user.status === "pending") {
      return res.status(403).json({ success: false, message: "Account pending superadmin approval." });
    }
    ...
    await pool.query("INSERT INTO refresh_tokens (admin_id, token, expires_at) VALUES ($1, $2, NOW() + INTERVAL '7 days')", [user.id, refreshToken]);

    const { password_hash, ...safeUser } = user;
    return res.status(200).json({ success: true, message: "Admin login successful", user: safeUser, accessToken, refreshToken });
  } catch (error) {
...
export const verifyOTP = async (req, res) => {
  ...
  try {
    const userRes = await pool.query("SELECT id FROM admins WHERE email = $1", [email.toLowerCase()]);
    if (userRes.rows.length === 0) return res.status(404).json({ success: false, message: "User not found" });
    const user = userRes.rows[0];

    const otpRes = await pool.query("UPDATE otps SET is_used = TRUE WHERE admin_id = $1 AND otp = $2 AND is_used = FALSE AND expires_at > NOW() RETURNING id", [user.id, otp]);
    if (otpRes.rows.length === 0) return res.status(400).json({ success: false, message: "Invalid/Expired OTP" });

    await pool.query("UPDATE admins SET is_verified = TRUE WHERE id = $1", [user.id]);
    return res.status(200).json({ success: true, message: "Email verified. Await superadmin approval." });
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    const userRes = await pool.query("SELECT id FROM admins WHERE email = $1 AND role = 'admin'", [email.toLowerCase()]);
    if (userRes.rows.length > 0) {
      const otp = generateOTP();
      await pool.query("INSERT INTO otps (admin_id, otp, expires_at) VALUES ($1, $2, NOW() + INTERVAL '10 minutes')", [userRes.rows[0].id, otp]);
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    const otpRes = await pool.query("UPDATE otps SET is_used = TRUE WHERE admin_id = $1 AND otp = $2 AND is_used = FALSE AND expires_at > NOW() RETURNING id", [userRes.rows[0].id, otp]);
    if (otpRes.rows.length === 0) return res.status(400).json({ success: false, message: "Invalid/Expired OTP" });

    const hashedPassword = await hashPassword(newPassword);
    await pool.query("UPDATE admins SET password_hash = $1 WHERE id = $2", [hashedPassword, userRes.rows[0].id]);
...
export const refreshTokenController = async (req, res) => {
  ...
  try {
    ...
    if (!decoded) return res.status(401).json({ success: false, message: "Invalid/Expired token" });
    const userRes = await pool.query("SELECT id, email, role, status FROM admins WHERE id = $1 AND role = 'admin'", [decoded.id]);
    const user = userRes.rows[0];
  ...
  try {
    const result = await pool.query(
      `SELECT u.id, u.first_name AS "firstName", u.last_name AS "lastName", u.email, u.mobile, u.role, u.status, p.bio, p.designation, p.organization
       FROM admins u LEFT JOIN profiles p ON u.id = p.admin_id WHERE u.id = $1`, [req.user.id]
    );
...
export const updateProfile = async (req, res) => {
  ...
  try {
    await pool.query("UPDATE admins SET first_name = $1, last_name = $2, mobile = $3 WHERE id = $4", [firstName, lastName, mobile, req.user.id]);
    await pool.query(
